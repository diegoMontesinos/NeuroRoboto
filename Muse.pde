...
public class Muse {
  ...
  // Buffer and average
  private int BUFFER_SIZE  = 30000;
  private int AVERAGE_SIZE = 24;
  ...
  private float getAverageSignal (ArrayList<Float> signal, float defaultVal) {
    int count = signal.size();
    if (count == 0) return defaultVal;
    if (count < AVERAGE_SIZE) return defaultVal;

    float acc = 0.0;
    for (int i = count - AVERAGE_SIZE; i < count; i++) {
      acc += signal.get(i);
    }

    return acc / float(AVERAGE_SIZE);
  }
  ...
  public float getAverageConcentration () {
    return this.getAverageSignal(this.concentration, 100.0);
  }
  ...
  public float getAverageMellow () {
    return this.getAverageSignal(this.mellow, 0.0);
  }
  ...
  public float getAverageAlpha () {
    return this.getAverageSignal(this.alpha, 0.0);
  }
  ...
  public float getAverageBeta () {
    return this.getAverageSignal(this.beta, 0.0);
  }
  ...
  public float getAverageDelta () {
    return this.getAverageSignal(this.delta, 0.0);
  }
  ...
  public float getAverageTheta () {
    return this.getAverageSignal(this.theta, 0.0);
  }
  ...
  public float getAverageGamma () {
    return this.getAverageSignal(this.gamma, 0.0);
  }
  ...
  public void drawAverageSignals (float x, float y, float w, float h, float transpa) {
    this.drawAverageSignal(this.delta, -1, 1, x, y, w, h, color(255, 0, 0, transpa));
    this.drawAverageSignal(this.theta, -1, 1, x, y, w, h, color(242, 26, 206, transpa));
    this.drawAverageSignal(this.alpha, -1, 1, x, y, w, h, color(29, 231, 249, transpa));
    this.drawAverageSignal(this.beta, -1, 1, x, y, w, h, color(22, 224, 42, transpa));
    this.drawAverageSignal(this.gamma, -1, 1, x, y, w, h, color(255, 155, 33, transpa));
    this.drawAverageSignal(this.mellow, 0, 100, x, y, w, h, color(22, 67, 249, transpa));
    this.drawAverageSignal(this.concentration, 0, 100, x, y, w, h, color(255, 230, 0, transpa));
  }
  ...
  private void drawAverageSignal (ArrayList<Float> signal, float min, float max, float x, float y, float w, float h, int c) {
    int avgCount = 12;
    int len = signal.size();
    float sepa = w / (float(len) / float(avgCount));
    ...
    float value;
    float signalX = 0.0, signalY;
    for (int i = 0; i < len; i += avgCount) {
